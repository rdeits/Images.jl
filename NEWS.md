...
- `imcorner` accepts the `method` as a positional rather than keyword
  argument. The `method` is the first argument, in order to support
  Julia's `do`-block syntax. The old syntax will issue a deprecation warning.
